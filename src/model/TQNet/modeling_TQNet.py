...
from .configuration_TQNet import TQNetConfig

@dataclass
class TQNetOutput(ModelOutput):
    pred: torch.Tensor | None = None
    loss: torch.Tensor | None = None

class TQNetLoss(nn.Module):
    def __init__(self, config: TQNetConfig):
        super().__init__()
        self.config = config
...
class TQNet(PreTrainedModel):
    def __init__(self, configs: TQNetConfig):
        ...
        self.criterion = TQNetLoss(configs)
    ...
    def forward(self, timeseries: torch.Tensor, labels: torch.Tensor | None = None, timestamp: torch.Tensor | None = None, **kwargs) -> TQNetOutput:
        cycle_index = timestamp.to(dtype=torch.int64) % self.cycle_len
        x = timeseries
        # instance norm
        if self.use_revin:
        ...
        return TQNetOutput(pred=output, loss=loss)
