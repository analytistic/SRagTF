...
from .modeling_module import Encoder, EncoderLayer, FullAttention, AttentionLayer
import numpy as np
...
class iTransformer_Loss(nn.Module):
...
class iTransformer(PreTrainedModel):
    """
    ...
    def __init__(self, configs: iTransformerConfig):
        ...
        self.criterion = iTransformer_Loss(configs)
    ...
    def forward(self, timeseries, labels):
        dec_out, attns = self.forecast(timeseries, None, None, None)
        ...
        if self.output_attention:
            ...
            return iTransformerOutput(
                loss=loss,
                pred=dec_out[:, -self.pred_len:, :],  # [B, L, D]
                attn=attns
            )
        else:
            return iTransformerOutput(
